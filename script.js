const socket = io.connect("http://localhost:3500", {
  ...
  function displayUploadedImage(image) {
    ...
    // Implementasikan logika pengiriman gambar ke server atau chat jika diperlukan
    // (Anda dapat mengirim URL gambar ke server atau menampilkan gambar dalam pesan chat).
  }
  const emojiButton = document.getElementById('emojiButton');
const messageInput = document.getElementById('message'); // Ganti dengan ID input pesan Anda

emojiButton.addEventListener('click', () => {
messageInput.focus();
document.execCommand('insertText', false, '😎😎😎😎'); // Ganti ini dengan emoji lokal yang Anda inginkan
});
